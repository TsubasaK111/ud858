...
function enableButtons () {
  ...
  // Set the onclick action for the third button
	...
	// Update the button label now that the button is active
	btn.value="Click me for a timely greeting";
}
...
function greetByPeriod () {
  ...
  var period = document.getElementById("period_field").value;
	...
	// Call the sayHelloByName() function. It takes one argument "name"
	// On success, pass the response to sayHelloCallback()
	var request = gapi.client.helloworldendpoints.\
                  greetByPeriod({'name': name, 'period': period });
	request.execute(sayHelloCallback);
}
